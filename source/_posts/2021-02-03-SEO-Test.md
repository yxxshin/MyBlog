...
{% blockquote %}
Desilo 를 Google에 검색하였을 때, 내 블로그의 인턴 후기 게시물이 첫 페이지에 노출된다!
{% endblockquote %}

![블로그 검색 결과](../images/post_images/Blog_search_result.png)

SEO가 잘 작동한다는 것은 좋은 소식이나, 사실 게시물을 작성하기 전부터 내 인턴 후기가 의도치 않은 구글 검색에 노출되는 것은 원하지 않았다. 예를 들어 Desilo 회사에 대하여 알아보기 위해 검색했는데, 내 블로그 게시물이 검색 결과에 노출되어 읽어보는 상황이 있겠다. 나쁜 말을 쓴 것은 결코 아니지만, 어쨌든 회사의 사람들과 합의를 하거나 검사를 받고 올린 글은 아니지 않은가. (Desilo에 관한 인터뷰를 본인에게 직접 요청한 사람도 있었다. 이 사람도 이러한 경로로 나에게 찾아오지 않았을까?)
...
허나 문제는, `sitemap: false`를 추가하기 전에 이미 블로그를 한 번 서버에 업로드를 하였고, 초기의 sitemap 버전에 인턴 후기 글들이 모두 실렸으며, Google-bot이 초기 sitemap을 통해 이미 크롤링을 다 해버렸으며, 따라서 **그 이후에 sitemap에서 페이지를 삭제한 것은 전혀 유효하지 않았다**. 
...
원하는 방향으로 모두 잘 작동하였다. 원하는 두 페이지에만 noindex 메타 태그가 삽입되었으며, HTML 파일이 생성되는 본인의 [Github](https://github.com/yxxshin/yxxshin.github.io) 이나 Chrome의 F12 DevTools(Elements)를 통해 이를 확인할 수 있었다. 허나, sitemap의 update가 이루어졌음에도 검색 결과에서 본인의 블로그가 삭제되지 않았다. **대 실패**였다. 

결국 최선의 방안을 선택하게 되었다: 

---

메모: [Google Search Console Settings](https://search.google.com/search-console/remove-outdated-content)