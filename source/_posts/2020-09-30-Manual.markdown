...
`node_modules/kramed/lib/rules/inline.js` 에서 앞쪽 `var inline = { ... }` 부분에서, 다음의 코드를 집어넣고 기존의 `escape`과 `em`을 지워줍니다.
...
Latex에서 `_`가 아래 첨자로 사용되지만, Markdown에서 `_`도 bold나 italic 등에 사용되기 때문에 발생하는 문제입니다. 
